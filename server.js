...
async function getDailyEarningStatus(client, userId) {
  ...
  // Recovery for users/transactions created before the earning-session marker
  // existed. If there is no active marker, establish the current session from
  // the earliest qualifying non-CPX earning made in the last 24 hours.
  if (!sessionStart) {
    const recovered = await client.query(
      `SELECT MIN(created_at) AS first_earning
         FROM transactions
        WHERE user_id = $1
          AND amount > 0
          AND type = ANY($2::text[])
    ...
    if (recovered.rows[0]?.first_earning) {
      sessionStart = new Date(recovered.rows[0].first_earning);
    }
  }
