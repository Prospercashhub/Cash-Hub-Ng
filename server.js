...
app.post("/api/withdraw", async (req, res) => {
  ...
  try {
    ...
    if (value < 28000) { await client.query("ROLLBACK"); return res.status(400).json({ error: "Minimum withdrawal is ₦28,000." }); }
...
app.get("/api/adslab/postback", async (req, res) => {
  ...
  try {
    ...
    if (pid === ADSLAB_TASK_PLACEMENT_ID) {
      ...
      if (!Number.isFinite(providerAmount) || providerAmount <= 0) { await client.query("ROLLBACK"); inTransaction = false; return res.status(400).send("Invalid reward"); }
