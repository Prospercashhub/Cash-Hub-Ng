...
const app = express();
// ================= DAILY EARNING LIMIT =================
// Users can earn a maximum of ₦3,000 in one 24-hour earning session.
// This never resets or reduces the user's actual wallet balance.
...
async function getDailyEarningStatus(client, userId) {
  ...
  const first = result.rows[0]?.first_earning_at
    ? new Date(result.rows[0].first_earning_at)
    : null;
  const locked = earned >= DAILY_EARNING_LIMIT_NGN;
  const unlockAt = first && locked
    ? new Date(first.getTime() + 24 * 60 * 60 * 1000)
    : null;
  ...
  try {
    ...
    try {
      ...
      if (limitErr.code === "DAILY_EARNING_LIMIT_REACHED") {
        return res.status(429).json({ error: "Daily earning limit reached. Come back after 24 hours." });
      }
      throw limitErr;
    }
...
app.get("/api/cpx-callback", async (req, res) => {
  ...
  try {
    ...
    try {
      await enforceDailyEarningLimit(client, user_id, reward);
    } catch (limitErr) {
      await client.query("ROLLBACK");
      if (limitErr.code === "DAILY_EARNING_LIMIT_REACHED") return res.status(429).send("Daily earning limit reached. Come back after 24 hours.");
      throw limitErr;
    }
