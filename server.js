...
// ================= EARNWALL POSTBACK =================
const EARNWALL_SECRET = process.env.EARNWALL_SECRET || "YOUR_EARNWALL_SECRET";
const processedEarnWallRewards = new Set();
app.post("/api/earnwall/postback", async (req,res)=>{
  try{
    const {userId,reward,transactionId,signature}=req.body||{};
    const expected=crypto.createHmac("sha256",EARNWALL_SECRET).update(`${userId}:${reward}:${transactionId}`).digest("hex");
    if(!signature||signature!==expected) return res.status(403).json({error:"Invalid signature"});
    if(processedEarnWallRewards.has(transactionId)) return res.json({success:true,duplicate:true});
    processedEarnWallRewards.add(transactionId);
    // Existing wallet credit + transaction recording should be performed using the project's current Neon layer.
