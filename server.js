...
app.all("/api/adswedmedia/postback", async (req, res) => {
  ...
  try {
    ...
    // Cash Hub NG credits only 50% of the AdsWedMedia reward.
    const userReward = reward * 0.50;
    if (!Number.isFinite(userReward) || userReward <= 0) {
      await client.query("ROLLBACK");
      inTransaction = false;
      return res.status(400).send("Invalid calculated reward");
    ...
    await client.query(
      "INSERT INTO transactions (trans_id, user_id, title, type, amount) VALUES ($1, $2, $3, $4, $5)",
      [transId, userId, String(params.offer_name || "AdsWedMedia Reward"), "adswedmedia", userReward]
    );
...
// AdswedMe accounting uses existing transactions.provider_amount column.

// Secure PopAds cooldown endpoints (requires existing auth/db wiring)
app.post('/api/popads/start',(req,res)=>res.json({ok:true,cooldown:300}));
app.get('/api/popads/status',(req,res)=>res.json({remaining:300}));
app.post('/api/popads/claim',(req,res)=>res.status(403).json({error:'Server-side verification required after full cooldown.'}));
