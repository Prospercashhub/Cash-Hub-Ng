...
app.post("/api/withdraw", async (req, res) => {
  ...
  try {
    ...
    if (value < 1000) { await client.query("ROLLBACK"); return res.status(400).json({ error: "Minimum withdrawal is ₦1,000." }); }
    if (value > 15000) { await client.query("ROLLBACK"); return res.status(400).json({ error: "Maximum withdrawal is ₦15,000 per request." }); }
...
app.post("/api/complete-task", async (req, res) => {
  ...
  try {
    ...
    const taskTransId = `task_${taskId}_${Date.now()}`;
    await client.query("INSERT INTO transactions (trans_id, user_id, title, type, amount, provider_amount, cashhub_topup) VALUES ($1, $2, $3, $4, $5, $6, $7)", [taskTransId, userId, task.title, "earning", reward, reward, 0]);
    const updatedUser = await client.query(`SELECT balance, ${earningColumn} AS earnings FROM users WHERE id = $1`, [userId]);
    await client.query("COMMIT");
    return res.json({ ok: true, reward, balance: Number(updatedUser.rows[0]?.balance || 0), earnings: Number(updatedUser.rows[0]?.earnings || 0) });
  } catch (err) {
    await client.query("ROLLBACK").catch(() => {}); console.error(err); return res.status(500).json({ error: "Server Error" });
  } finally { client.release(); }
});
...
app.all("/api/adswedmedia/postback", async (req, res) => {
  ...
  try {
    ...
    // Cash Hub NG no longer adds a bonus/top-up to AdsWedMedia rewards.
    // Credit the verified provider reward as-is and keep cashhub_topup at zero.
    const providerAmount = reward;
    const cashhubTopup = 0;

    if (!Number.isFinite(providerAmount) || providerAmount <= 0) {
      await client.query("ROLLBACK");
      inTransaction = false;
      return res.status(400).send("Invalid provider reward");
    }

    const userReward = providerAmount;
...
app.get("/api/adslab/postback", async (req, res) => {
  ...
  try {
    ...
    // Rewarded ads are currently disabled in Cash Hub NG. The Earn page does not
    // expose a Watch Ads option, so do not credit legacy/external rewarded-ad callbacks.
    if (pid === ADSLAB_REWARDED_PLACEMENT_ID) return res.status(200).send("OK");
    ...
    // Only AdsLab task rewards are creditable. Rewarded-ad callbacks are
    // disabled above, so no legacy rewarded-ad reward variable is used.
    ...
    if (!/^(?:0|[1-9]\d*)(?:\.\d+)?$/.test(rewardRaw)) {
      await client.query("ROLLBACK"); inTransaction = false; return res.status(400).send("Invalid reward");
    }

    const providerAmount = Number(rewardRaw);
    if (!Number.isFinite(providerAmount) || providerAmount <= 0) {
      await client.query("ROLLBACK"); inTransaction = false; return res.status(400).send("Invalid reward");
    }

    // Cash Hub NG credits the verified AdsLab provider reward as-is.
    const userReward = providerAmount;
    const cashhubTopup = 0;
    const title = String(req.query.name || "AdsLab Task Reward").slice(0, 200);
    const type = "adslab_task";
    try {
