...
async function renderRecentTransactions(transactions) {
  ...
  container.innerHTML = transactions
    ...
    .map(t => {
      const date = new Date(
        t.created_at || t.createdAt || t.date || Date.now()
      ).toLocaleString();
      ...
      return `
        <div class="transaction">
          <div>
            <strong>${t.title || t.type || "Activity"}</strong>
            <small>${date}</small>
          </div>

          <b class="${amount >= 0 ? "positive" : "negative"}">
...
async function loadDashboard() {
  try {
    ...
    const refCount =
      document.getElementById("dashboard-ref-count");
    ...
    if (userEarnings) {
      userEarnings.textContent = money(user.earnings);
    }

    if (userReferral) {
      userReferral.textContent =
        money(
