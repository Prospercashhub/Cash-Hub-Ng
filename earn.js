...
window.openEarnWall = async function(){
 const session=getSession();
 if(!session||!session.id){ alert("Please login first."); return; }
 // Pass the logged-in user's UUID to the existing EarnWall integration.
 const uid=encodeURIComponent(session.id);
 window.location.href=`earn.html?userId=${uid}`;
};
